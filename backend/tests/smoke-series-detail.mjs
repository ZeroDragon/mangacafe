...
const sB = await series.create(uB.id, {
  type: 'anime', name: 'Serie B', url: null, cover_url: null, imdb_url: null
})
...
log('Verificar que last_read = "Cap 101" (el único visto)')
const afterSeen101 = await request('get', `/api/series/${s1.id}`, null, tokenA)
if (afterSeen101.data.data.last_read !== 'Cap 101') fail(`last_read debería ser 'Cap 101', vino '${afterSeen101.data.data.last_read}'`)
...
let s2series = (await request('get', `/api/series/${s2.id}`, null, tokenA)).data.data
if (s2series.last_read !== 'Cap 3') fail(`last_read debería ser 'Cap 3' (último visto), vino '${s2series.last_read}'`)
...
log('  cascada seen OK (1,2,3 vistos | 4,5 pendientes | last_read=Cap 3)')
...
s2series = (await request('get', `/api/series/${s2.id}`, null, tokenA)).data.data
if (s2series.last_read !== 'Cap 1') fail(`last_read debería ser 'Cap 1' (único visto tras desmarcar 2,3), vino '${s2series.last_read}'`)
...
log('  cascada unseen OK (1 visto | 2,3,4,5 pendientes | last_read=Cap 1)')
...
s2series = (await request('get', `/api/series/${s2.id}`, null, tokenA)).data.data
if (s2series.last_read !== 'Cap 5') fail(`last_read debería ser 'Cap 5' (último cronológico), vino '${s2series.last_read}'`)
...
log('  cascada seen hasta el último OK (last_read=Cap 5)')
...
s2series = (await request('get', `/api/series/${s2.id}`, null, tokenA)).data.data
if (s2series.last_read !== null) fail(`last_read debería ser null (sin vistos -> 'No data'), vino '${s2series.last_read}'`)
...
log('  cascada unseen desde el primero OK (last_read=null -> No data)')
